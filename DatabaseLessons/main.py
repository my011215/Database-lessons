...
s = SQL(user='xxxx', host="127.0.0.1", password='xxxxx', db='xxxx',)
...
def hot_spot_cul():
    db = s.conn()
    cursor = db.cursor()
    sql = '''SELECT title,comment_count,source,type FROM `essay_information` ORDER BY comment_count DESC;'''
    cursor.execute(sql)
    ...
    for i in range(len(essays)):
        try:
            if max_comment != essays[i][1]:
                j += x
                max_comment = essays[i][1]
                x = 1
            else:
                x += 1
            sql = f'''INSERT INTO `sjk`.`hot_spot_information` (`id`,`name`,`comment_count`,`source`,`type`,`rank`)
                  VALUES ({str(i+1)}, '{essays[i][0]}',{essays[i][1]},'{essays[i][2]}','{essays[i][3]}',{j})'''
            cursor = db.cursor()
            cursor.execute(sql)
        except Exception as e:
            print(e, e.__class__.__name__)
